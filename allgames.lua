...
_G.UnknownConfig = {
    ...
    AimlockEnabled = true,
    AimlockSmoothness = 0.5, -- Giá trị càng nhỏ càng ghim chặt
...
-- ==================== CONTROL PANEL TRONG HEADER ====================
local function createControlButtons()
    local ControlFrame = Instance.new("Frame")
    ControlFrame.Name = "ControlPanel"
    ControlFrame.Size = UDim2.new(0, 195, 0, 48)
    ControlFrame.Position = UDim2.new(1, -210, 0, 8)
    ...
    local stroke = Instance.new("UIStroke", ControlFrame)
    ...
    ToggleBtn.MouseButton1Click:Connect(function()
        ...
        if FloatingButton then
            FloatingButton.Visible = not scriptEnabled
        end
    end)
    ...
    DestroyBtn.MouseButton1Click:Connect(function()
        if FloatingButton then FloatingButton:Destroy() end
        ScreenGui:Destroy()
        print("🌌 Unknown Hub đã bị xóa hoàn toàn.")
    end)
